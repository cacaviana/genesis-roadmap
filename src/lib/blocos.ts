...
export const blocosFuturo: Bloco[] = [
  ...
  },
  {
    id: 'api-novo',
    nome: '✅ Backend FastAPI',
    tag: 'Python · ATIVO em PROD',
    resumo: 'PROD rodando com 2 workers Fase 2 dentro (status + inbound). webhook + campanha já isolados.',
    variante: 'ok',
    ondeMora: 'Azure App Service · `genesisbackendd`',
    oQueFaz: 'Recebe HTTP do frontend, valida, publica msg no Service Bus, retorna 202. Roda status_updater_worker + inbound_processor_worker. webhook_worker e campanha_worker em App Services dedicados.',
    recebeDe: ['Frontend (HTTP)', 'Webhook Meta (ainda aqui)', 'Tópicos messaging.status + messaging.inbound'],
    entrega: ['Service Bus filas', 'UPDATE mensagens.status + broadcast WS'],
    payloadEntrada: {
...
// 2. Resposta HTTP 202 imediata pro frontend
{
  ...
  },
  {
    ...
    tag: 'Python · NÃO ATIVADO',
    resumo: 'PENDENTE — hoje campanha vai via genesis-campanhas (legacy). Refactor pra usar campaigns.dispatch.',
    variante: 'warn',
    ondeMora: 'Hoje: lógica está em worker-genesis-campanhas chamando CampanhaService.disparar com flag MESSAGING_SERVICE_ENABLED',
...
// Header: SessionId = "cliente-42"  (ordem por tenant)`,
  ...
  {
    id: 'msg-service',
    nome: '⚠️ messaging-service (fork meu, não da Polly)',
    tag: 'Python · ATIVO mas em fork',
    resumo: 'Rodando em messaging-service-sandbox com tokens reais. Polly precisa mergear fork no repo dela + adicionar multi-tenancy.',
    variante: 'warn',
    ondeMora: 'Azure App Service · `messaging-service-sandbox` (fork cacaviana/messaging-service)',
...
// 3. Publica evento em messaging.status:
{
  ...
  },
  {
    ...
    tag: 'Service Bus Queue · VAZIA',
    resumo: 'Criada mas NÃO USADA. Hoje campanha vai via genesis-campanhas (legacy). Refactor pendente.',
    variante: 'warn',
...
// Depois: await receiver.complete_message(msg)`,
  ...
  },
  {
    ...
    tag: 'Service Bus Queue · EM USO',
    resumo: 'ATIVA em PROD. Genesis publica, messaging-service consome. Smoke test E2E real validou.',
    variante: 'ok',
    ondeMora: 'Azure Service Bus Premium',
...
// 5. complete_message(msg)`,
  ...
  },
  {
    ...
    tag: 'Service Bus Topic · EM USO',
    resumo: 'ATIVO em PROD. messaging-service publica. Genesis status_updater_worker (PROD) consome.',
    variante: 'ok',
    ondeMora: 'Azure Service Bus Premium',
...
ws.broadcast(tenant_id, {
  ...
  },
  {
    ...
    tag: 'Service Bus Topic · VAZIO',
    resumo: 'Criado MAS sem publishers ainda. Webhook Meta ainda aponta pro Genesis direto. Pra ativar: trocar webhook URL no Meta Business Manager.',
    variante: 'warn',
    ondeMora: 'Azure Service Bus Premium',
...
// Adicionar Telegram = só messaging-service muda, Genesis nem percebe`,
  ...
  },
  {
    ...
    tag: 'Python · ATIVO em PROD',
    resumo: 'RODANDO em PROD desde 2026-05-23 00:50 UTC. Smoke real validou.',
    variante: 'ok',
    ondeMora: 'Dentro do `genesisbackendd` em PROD (não tem App Service dedicado ainda)',
...
ws.broadcast(tenant_id="cliente-42", {
  ...
  {
    id: 'worker-inbound',
    nome: '⚠️ Worker Inbound Processor',
    tag: 'Python · ATIVO mas SEM TRABALHO',
    resumo: 'Rodando em PROD, mas messaging.inbound topic ainda vazio. Só vai processar quando webhook Meta apontar pro messaging-service.',
    variante: 'warn',
    ondeMora: 'Dentro do `genesisbackendd` em PROD',
